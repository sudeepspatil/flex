...
fetch(`https://api.themoviedb.org/3${movie_id}?` + new URLSearchParams({
    api_key : api_key
}))
.then(res => res.json())
.then(data => {
...
fetch(`https://api.themoviedb.org/3${movie_id}/credits?` + new URLSearchParams({
    api_key: api_key
}))
.then(res => res.json())
.then(data => {
...
fetch(`https://api.themoviedb.org/3${movie_id}/videos?` + new URLSearchParams({
    api_key:api_key
}))
.then(res => res.json())
.then(data => {
...
fetch(`https://api.themoviedb.org/3${movie_id}/recommendations?` + new URLSearchParams({
    api_key: api_key
}))
.then(res => res.json())
.then(data =>{
    ...
    for(let i = 0;i < 16; i++){
        ...
        container.innerHTML +=`
        <div class="movie" onclick="location.href = '/movie/${data.results[i].id}'">
